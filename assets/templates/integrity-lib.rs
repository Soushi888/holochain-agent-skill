    ...
    match op.flattened::<EntryTypes, LinkTypes>()? {
        FlatOp::CreateEntry(create_entry) => match create_entry {
            OpEntry::CreateEntry { app_entry, action } => {
                ...
                match app_entry {
                    EntryTypes::<EntryType>(<entry_type>) => validate_create_post(create_action, <entry_type>),
                }
            }
            OpEntry::UpdateEntry {
                app_entry, action, ..
            ...
            OpUpdate::Entry { app_entry, action } => {
                ...
                match app_entry {
                    EntryTypes::<EntryType>(<entry_type>) => {
                        ...
                        let original_post = match <EntryType>::try_from(original_app_entry) {
                            Ok(entry) => entry,
                            Err(e) => {
                        ...
                        validate_update_post(action, <entry_type>, original_action, original_post)
                    }
                }
            }
            _ => Ok(ValidateCallbackResult::Valid),
        ...
        FlatOp::Delete(OpDelete { action }) => {
            ...
            match original_app_entry {
                EntryTypes::<EntryType>(original_post) => {
        ...
        FlatOp::CreateRecord(store_record) => {
            match store_record {
                ...
                OpRecord::CreateEntry { app_entry, action } => {
                    ...
                    match app_entry {
                        EntryTypes::<EntryType>(<entry_type>) => validate_create_post(action, <entry_type>),
                    }
                }
                ...
                OpRecord::UpdateEntry {
                    ...
                    match app_entry {
                        EntryTypes::<EntryType>(<entry_type>) => {
                            let result = validate_create_post(creation_action, <entry_type>.clone())?;
                            if let ValidateCallbackResult::Valid = result {
                                let original_post: Option<<EntryType>> = original_record
                                    .entry()
                                    .to_app_option()
